 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
const X = globalThis, he = X.trustedTypes, Qe = he ? he.createPolicy("lit-html", { createHTML: (i) => i }) : void 0, yt = "$lit$", P = `lit$${Math.random().toFixed(9).slice(2)}$`, bt = "?" + P, Gt = `<${bt}>`, R = document, W = () => R.createComment(""), K = (i) => i === null || typeof i != "object" && typeof i != "function", Ge = Array.isArray, Tt = (i) => Ge(i) || typeof (i == null ? void 0 : i[Symbol.iterator]) == "function", ke = `[ 	
\f\r]`, q = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, et = /-->/g, tt = />/g, E = RegExp(`>|${ke}(?:([^\\s"'>=/]+)(${ke}*=${ke}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), rt = /'/g, it = /"/g, mt = /^(?:script|style|textarea|title)$/i, Zt = (i) => (e, ...t) => ({ _$litType$: i, strings: e, values: t }), a = Zt(1), B = Symbol.for("lit-noChange"), b = Symbol.for("lit-nothing"), ot = /* @__PURE__ */ new WeakMap(), A = R.createTreeWalker(R, 129);
function _t(i, e) {
...
const It = (i, e) => {
  ...
  for (let u = 0; u < t; u++) {
    const c = i[u];
    let v, y, g = -1, m = 0;
    for (; m < c.length && (n.lastIndex = m, y = n.exec(c), y !== null); ) m = n.lastIndex, n === q ? y[1] === "!--" ? n = et : y[1] !== void 0 ? n = tt : y[2] !== void 0 ? (mt.test(y[2]) && (r = RegExp("</" + y[2], "g")), n = E) : y[3] !== void 0 && (n = E) : n === E ? y[0] === ">" ? (n = r ?? q, g = -1) : y[1] === void 0 ? g = -2 : (g = n.lastIndex - y[2].length, v = y[1], n = y[3] === void 0 ? E : y[3] === '"' ? it : rt) : n === it || n === rt ? n = E : n === et || n === tt ? n = q : (n = E, r = void 0);
    const h = n === E && i[u + 1].startsWith("/>") ? " " : "";
    s += n === q ? c + Gt : g >= 0 ? (o.push(v), c.slice(0, g) + yt + c.slice(g) + P + h) : c + P + (g === -2 ? u : h);
  }
    ...
    for (; (r = A.nextNode()) !== null && c.length < u; ) {
      if (r.nodeType === 1) {
        if (r.hasAttributes()) for (const g of r.getAttributeNames()) if (g.endsWith(yt)) {
          ...
          c.push({ type: 1, index: s, name: _[2], strings: h, ctor: _[1] === "." ? Nt : _[1] === "?" ? Ft : _[1] === "@" ? qt : ye }), r.removeAttribute(g);
...
let D = class extends p {
  ...
  attributeChangedCallback(i, e, t) {
    switch (super.attributeChangedCallback(i, e, t), i) {
      case "ripple":
        this.rippleCleanUp !== null && (this.rippleCleanUp(), this.rippleCleanUp = null), t !== null && (this.rippleCleanUp = be(this));
        break;
    }
  }
};
...
var bi = Object.defineProperty, mi = Object.getOwnPropertyDescriptor, qe = (i, e, t, o) => {
...
let te = class extends p {
  ...
  async enableRipple() {
    ...
    i && (this.rippleCleanUp = be(i), this.role = "button", this.style.cursor = "pointer", this.addEventListener("click", this.clickHandler), [...this.children].forEach((t) => {
