...
class C {
  constructor(e) {
    this.serverPathPrefix = "", this.socket = null, this.events = new E(), this.reconnectInterval = 1e3, this.timeout = null, this.open = !1, this.onClose = () => {
      this.timeout !== null && (clearTimeout(this.timeout), this.timeout = null), this.open && (this.open = !1, this.events.dispatch("close", this)), this.timeout = setTimeout(async () => {
        console.debug(`WS: try to reconnect to "${this.getURL()}"`), await this.connect();
      }, this.reconnectInterval);
    }, this.onOpen = () => {
      console.debug(`WS: connected to "${this.getURL()}"`), this.open || (this.open = !0, this.events.dispatch("open", this));
    }, this.onMessage = async (t) => {
      const s = JSON.parse(await t.data.text());
      console.debug("WS: Got a message:", s), this.events.dispatch("message", s);
    }, this.serverPathPrefix = e;
  }
  getURL() {
  ...
  async connect() {
    ...
    const e = this.getURL();
    console.debug(`Try to connect WebSocket to ${e}`), this.socket = new WebSocket(e), this.socket.addEventListener("close", this.onClose), this.socket.addEventListener("open", this.onOpen), this.socket.addEventListener("message", this.onMessage);
  }
  close() {
...
function S(r, e, t) {
  ...
  return e.draggable = !0, e.ondragstart = (n) => {
    ...
    n.preventDefault(), [...r.children].forEach((i, l) => {
      if (l === s) {
...
function w(r, e) {
  e = { ...L, ...e };
  let t = null, s = null, n = null, i = !1, l = "", u = "";
  const v = (o) => {
    const c = o.currentTarget;
    !t && Array.from(c.classList).includes("draggable") && (s = (/* @__PURE__ */ new Date()).getTime(), t = c, n && clearTimeout(n), n = setTimeout(() => {
      t && (l = t.style.color, u = t.style.backgroundColor, t.style.color = "var(--ui-primary-text)", t.style.backgroundColor = "var(--ui-primary)", r.classList.add("ui-dragging"), i = !0, e != null && e.onDragStart && e.onDragStart());
    }, 200));
  }, p = (o) => {
    if (!s || !t) return;
    ...
    o.preventDefault(), i || (i = !0, e != null && e.onDragStart && e.onDragStart());
    let c;
    if (window.TouchEvent && o instanceof TouchEvent) {
      const h = o.targetTouches[0];
      c = {
        clientX: h.clientX,
        clientY: h.clientY
      };
    ...
    let a = document.elementFromPoint(c.clientX, c.clientY);
    if (a) {
    ...
    n !== null && (clearTimeout(n), n = null), t && (t.style.color = l, t.style.backgroundColor = u, t = null), s = null, r.classList.remove("ui-dragging"), i && (i = !1, e != null && e.onDragEnd && e.onDragEnd());
  };
  return [...r.children].forEach((o) => {
    o.classList.add("draggable"), o.onmousedown = v, o.ontouchstart = v, o.onmousemove = p, o.ontouchmove = p, r.onmouseleave = o.onmouseup = f, o.ontouchend = f;
  }), () => {
    [...r.children].forEach(
      (o) => {
        o.classList.remove("draggable"), o.onmousedown = null, o.ontouchstart = null, o.onmousemove = null, o.ontouchmove = null, r.onmouseleave = o.onmouseup = null, o.ontouchend = null;
      }
    );
  };
}
...
function $(r, e) {
  ...
  let t = null;
  const s = (u) => {
    t = g(u, e), r.addEventListener("pointermove", i);
  }, n = () => {
    r.removeEventListener("pointermove", i), m(t), t = null;
  }, i = () => n(), l = (u) => {
    t = g(u, e), m(t), t = null;
  };
  return r.classList.add("ripple-container"), r.style.overflow = "hidden", e.usePointer ? (r.addEventListener("pointerdown", s), r.addEventListener("pointerup", n), r.addEventListener("pointerleave", n)) : r.addEventListener("click", l), () => {
