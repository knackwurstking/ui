...
`;let ze=class{constructor(t){this.root=t,this.events=new H,this.input=this.root.shadowRoot.querySelector("input"),this.input.type=this.root.getAttribute("type")||"text"}set type(t){this.input.type=t}get type(){return this.input.type||"text"}set value(t){this.input.value=t}get value(){switch(this.input.type){case"number":return new Number(this.input.value);default:return this.input.value}}set placeholder(t){this.input.placeholder=t}get placeholder(){return this.input.placeholder}set invalid(t){this.input.ariaInvalid=t?"":null}get invalid(){return this.input.ariaInvalid!==null}set min(t){this.input.min=t}get min(){switch(this.input.type){case"number":return new Number(this.input.min);default:return this.input.min}}set max(t){this.input.max=t}get max(){switch(this.input.type){case"number":return new Number(this.input.max);default:return this.input.max}}};const P=class P extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.appendChild(de.content.cloneNode(!0)),this.ui=new ze(this)}attributeChangedCallback(t,e,s){switch(t){case"type":s===null?this.ui.type="text":this.ui.type=s;break;case"value":this.ui.value=this.parseNewValue(s);break;case"placeholder":s===null?this.ui.placeholder="":this.ui.placeholder=s;break;case"invalid":this.ui.invalid=s!==null;break;case"min":this.ui.min=this.parseNewValue(s);break;case"max":this.ui.max=this.parseNewValue(s);break}}parseNewValue(t){switch(this.ui.type){case"number":return new Number(t);default:return t||""}}};o(P,"register",()=>customElements.define("ui-input",P)),o(P,"observedAttributes",["type","value","placeholder","invalid","min","max"]);let ot=P;const Ue=a`
<style>
    :host {
        display: none;
        align-items: center;
...
`,St=class St extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=We}};o(St,"register",()=>customElements.define("ui-svg-close",St));let Vt=St;const Fe=a`
<style>
    :host {
        width: 100%;
        height: 100%;
...
`,At=class At extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=Fe}};o(At,"register",()=>customElements.define("ui-svg-delete-recycle-bin",At));let Zt=At;const Ge=a`
<style>
    :host {
        width: 100%;
        height: 100%;
